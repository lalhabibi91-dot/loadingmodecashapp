(() => {
  ...
  const MODE_LABELS = { '1': 'Classic', '2': 'Pay Sheet', '4': 'White Modern' };
  ...
  function applySelectedMode(mode) {
    if (!['1','2','4'].includes(String(mode))) return false;
  ...
  function showModePicker() {
    ...
    // Mode 4 is the default for this build. Keep the existing mode picker/login
    // flow intact; only the initial mode selection is fixed to Mode 4.
    const stored = (() => { try { return localStorage.getItem(MODE_KEY); } catch (_) { return null; } })();
    const initialMode = ['1','2','4'].includes(stored) ? stored : DEFAULT_MODE;
  ...
  function showCredentialLogin(message = '') {
    ...
    closeAdmin();
    const stored = (() => { try { return localStorage.getItem(MODE_KEY); } catch (_) { return null; } })();
    applySelectedMode(['1','2','4'].includes(stored) ? stored : DEFAULT_MODE);
    if (modePicker) modePicker.classList.add('hidden');
